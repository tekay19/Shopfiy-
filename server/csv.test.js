...
const SAMPLE_CSV = [
  'Handle,Title,Body (HTML),Vendor,Type,Tags,SEO Title,SEO Description,Option1 Name,Option1 Value,Option2 Name,Option2 Value,Variant SKU,Variant Price,Variant Compare At Price,Variant Inventory Qty,Image Src',
  'red-mug,Red Mug,<p>A mug.</p>,Acme,Mugs,"kitchen, gift",Red Mug SEO,Buy the red mug,Color,Red,Size,Small,MUG-RED,9.99,14.99,25,https://example.com/red-mug-1.jpg',
  'red-mug,,,,,,,,Color,Blue,Size,Large,MUG-BLUE,9.99,14.99,10,https://example.com/red-mug-2.jpg',
...
test('groups variant rows under one product by handle', () => {
  ...
  assert.deepEqual(mug.optionNames, ['Color', 'Size']);
});
...
test('handles a single-variant product', () => {
  ...
  assert.deepEqual(hat.optionNames, []);
});
...
test('captures optionNames in order, skipping option columns blank across all rows', () => {
  const csv = [
    'Handle,Title,Option1 Name,Option1 Value,Option2 Name,Option2 Value,Option3 Name,Option3 Value',
    'multi,Multi,Color,Red,,,Material,Cotton',
    'multi,,Color,Blue,,,Material,Wool',
  ].join('\n');
  const products = parseProductsCsv(csv);
  assert.deepEqual(products[0].optionNames, ['Color', 'Material']);
});
